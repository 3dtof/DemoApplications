...
bool DepthCamera::setCameraProfile(const String &cameraProfileName)
{
  ...
  if(!configFile.getCameraProfile(cameraProfileName, config))
  {
  ...
  if(!config->getConfigSet("params", params))
  {
    logger(LOG_ERROR) << "DepthCamera: Could not find parameters to initialize profile '" << cameraProfileName << "'" << std::endl;
    return false;
  }
  ...
  for(auto i = 0; i < params->paramNames.size(); i++)
  {
    if(params->paramNames[i].compare(0, 2, "0x") == 0)
    {
      ...
      uint32_t value = (uint32_t)strtol(config->get("params", params->paramNames[i]).c_str(), &endptr, 0);
    ...
    if(bp)
    {
      if(!set(params->paramNames[i], config->getBoolean("params", params->paramNames[i])))
        return false;
    }
    else if(ip || ep)
    {
      if(!set(params->paramNames[i], config->getInteger("params", params->paramNames[i])))
        return false;
    }
    else if(up)
    {
      if(!set(params->paramNames[i], (uint)config->getInteger("params", params->paramNames[i])))
        return false;
    }
    else if(fp)
    {
      if(!set(params->paramNames[i], config->getFloat("params", params->paramNames[i])))
        return false;
    }
    else
    {
