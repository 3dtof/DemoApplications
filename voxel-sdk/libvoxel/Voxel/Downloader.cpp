...
bool USBDownloader::_configureForDownload()
{
  ...
  return _usbIO->controlTransfer(USBIO::TO_DEVICE, USBIO::REQUEST_VENDOR, USBIO::RECIPIENT_DEVICE, 0x05, 0, 0, buffer, 4, 5000);
}
...
bool USBDownloader::download(const String &file)
{
  ...
  return _configureForDownload() && _download(f, size);
}
