...
bool TintinEEPROMDownloader::_configureForDownload()
{
  ...
  if (d.productID() == TINTIN_CDK_PRODUCT_UVC)
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Reseting device..." << std::endl;
    VoxelXUProgrammer p({}, _device);
    p.reset();
  
    // Wait for 10 seconds
    logger(LOG_INFO) << "TintinEEPROMDownloader: Waiting for device reenumeration..." << std::endl;
    // _outStream << "TintinEEPROMDownloader: Waiting for device reenumeration..." << std::endl;
  ...
  if(!_usbIO->isInitialized())
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Could not open device" << std::endl;
    return false;
  }
      ...
      0x17, 0x00, 0x00, writeInit, 1))
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Failed to init write in EEPROM" << std::endl;
    return false;
  }
      ...
      0x1D, 0x00, 0x00, jedecID, 3))
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Failed to read JEDEC ID" << std::endl;
    return false;
  }
  
  logger(LOG_INFO) << "TintinEEPROMDownloader: EEPROM JEDEC ID = " << std::hex << "0x" << (uint)jedecID[0] << "0x" << (uint)jedecID[1] << "0x" << (uint)jedecID[2] << std::endl;
  // _outStream << "TintinEEPROMDownloader: EEPROM JEDEC ID = " << std::hex << "0x" << (uint)jedecID[0] << "0x" << (uint)jedecID[1] << "0x" << (uint)jedecID[2] << std::endl;;
  ...
  // _outStream << "TintinEEPROMDownloader: EEPROM Status = " << std::hex << "0x" << (uint)eepromStatus << std::endl;
    ...
    0x1E, 0x00, 0x00, writeInit, 1))
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Failed to enable write" << std::endl;
    return false;
  }
  ...
  // _outStream << "TintinEEPROMDownloader: Write enable, EEPROM Status = " << std::hex << "0x" << (uint)eepromStatus << std::endl;
    ...
    0x1E, 0x00, 0x00, eepromEraseCommand, 1))
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Failed to erase EEPROM" << std::endl;
    return false;
  }
  ...
  // _outStream << "TintinEEPROMDownloader: Erase, EEPROM Status = " << std::hex << "0x" << (uint)eepromStatus << std::endl;
  
  
  // Wait for 30 seconds
  logger(LOG_INFO) << "TintinEEPROMDownloader: Waiting for erase..." << std::endl;
  // _outStream << "TintinEEPROMDownloader: Waiting for erase..." << std::endl;
  ...
  // _outStream << "TintinEEPROMDownloader: Erase done, EEPROM Status = " << std::hex << "0x" << (uint)eepromStatus << std::endl;
...
bool TintinEEPROMDownloader::_getEEPROMStatus(uint8_t &eepromStatus)
{
    ...
    0x1B, 0x00, 0x00, eepromStat, 1))
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Failed to read EEPROM status" << std::endl;
    return false;
  }
...
bool TintinEEPROMDownloader::_printEEPROMFirst64Bytes()
{
    ...
    0x19, 0x00, 0x00, bytes, 64))
  {
    ...
    // _outStream << "TintinEEPROMDownloader: Failed to read EEPROM bytes" << std::endl;
    return false;
  }
...
bool TintinEEPROMDownloader::_download(InputFileStream &file, long unsigned int filesize)
{
 ...
 // _outStream << "TintinEEPROMDownloader: Starting EEPROM write..." << std::endl;
  ...
  while(bytesRemaining)
  {
      ...
      0x18, startAddress & 0xFFFF, (startAddress >> 16) & 0x00FF, dataReverse.data() + startAddress, bytesToSend))
    {
      ...
      // _outStream << "TintinEEPROMDownloader: Failed to write bytes at address 0x" << std::hex << startAddress << std::endl;
      return false;
    }
    ...
    if(i % 8092 == 0)
    {
      ...
      // _outStream << "TintinEEPROMDownloader: Pending " << std::dec << bytesRemaining << " bytes..." << std::endl;
    }
  ...
  // _outStream << "TintinEEPROMDownloader: EEPROM writes finished..." << std::endl;
