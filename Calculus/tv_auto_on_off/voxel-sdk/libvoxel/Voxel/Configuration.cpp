...
bool ConfigurationFile::isPresent(const String &section, const String &name) const
{
  ...
  if(set.params.find(name) == set.params.end())
    return false;
...
bool MainConfigurationFile::getCameraProfile(const String &profileName, ConfigurationFile *&cameraProfile)
{
  if(_cameraProfiles.find(profileName) != _cameraProfiles.end())
  {
    cameraProfile = &_cameraProfiles.at(profileName);
    return true;
  }
  return false;
}

bool MainConfigurationFile::getDefaultCameraProfile(ConfigurationFile *&defaultCameraProfile)
{
  return getCameraProfile(_defaultCameraProfileName, defaultCameraProfile);
}
